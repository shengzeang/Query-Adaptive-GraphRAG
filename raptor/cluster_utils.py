...
def global_cluster_embeddings(
    ...
    reduced_embeddings = umap.UMAP(
        n_neighbors=max(n_neighbors,2), n_components=dim, metric=metric
...
def get_optimal_clusters(
    ...
    bics = []
    for n in n_clusters:
        gm = GaussianMixture(n_components=n, random_state=random_state, reg_covar=1e-5)
        gm.fit(embeddings)
...
def GMM_cluster(embeddings: np.ndarray, threshold: float, random_state: int = 0):
    ...
    gm = GaussianMixture(n_components=n_clusters, random_state=random_state, reg_covar=1e-5)
    gm.fit(embeddings)
