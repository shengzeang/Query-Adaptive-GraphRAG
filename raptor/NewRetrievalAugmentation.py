...
class Builder:
    def __init__(self, treebuilder_config=None, tree=None):
        if treebuilder_config is None:
            treebuilder_config = BuilderConfig()
        if not isinstance(treebuilder_config, BuilderConfig):
            raise ValueError(
                "config must be an instance of TreeBuilderConfig"
            )
        ...
        tree_builder_class = supported_tree_builders[treebuilder_config.tree_builder_type][0]
        self.tree_builder = tree_builder_class(treebuilder_config.tree_builder_config)

        logging.info(
            f"Successfully initialized TreeBuilder with Config {treebuilder_config.log_config()}"
        )
...
class Retriever:
    def __init__(self, retriever_config=None, tree=None):
        if retriever_config is None:
            retriever_config = RetrieverConfig()
        if not isinstance(retriever_config, RetrieverConfig):
            raise ValueError(
                "config must be an instance of RetrieverConfig"
            )

        self.tree_retriever_config = retriever_config.tree_retriever_config
        self.qa_model = retriever_config.qa_model
        self.tree = tree
        ...
        if self.tree is not None:
            ...
            raise ValueError(
                "Tree must be given during initialization!"
            )

        logging.info(
            f"Successfully initialized Retriever with Config {retriever_config.log_config()}"
        )
    ...
    def answer_question(
        ...
        if return_layer_information:
            ...
            context = self.retrieve(
                    question, start_layer, num_layers, top_k, max_tokens, collapse_tree, False
                )
